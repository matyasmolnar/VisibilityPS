...
#                         : False - or the visibility amplitudes for the statistics

# if power spectrum statistics: True - deal with the complex power spectra
#                             : False - or the power spectra amplitudes for the statistics
...
if last_statistic_period < 20 or last_statistic_period > 60:
    raise ValueError(
        'Choose LAST averaging period to be between 20 and 60 seconds.')
...
# vis_data['bl'] # gives baselines
# vis_data['vis'] has shape (35, 1, 1024, 60) #(bls, JDs, chans, time ints)
...
def plot_single_bl_vis(data, time, day, bl):
    plt.figure(figsize=[10, 7])
    plt.plot(chan_range+1, data[find_nearest(last_dayflg[:, 0], time)[1], np.ndarray.tolist(
...
def factors(n):
    f = np.asarray(sorted(functools.reduce(list.__add__, ([i, n//i] for i in \
