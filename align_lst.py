...
class LST_Binner:
    ...
    def __init__(self,
        ...
        ind_range = numpy.argwhere(
            (lst_subset > self.lst_start) & (lst_subset < self.lst_end))
...
# rewrite this for visibilities
        ...
        for t in numpy.arange(visibilities.shape[0]):
            for d in numpy.arange(visibilities.shape[1]):
                for c in numpy.arange(visibilities.shape[3]):
                    ...
                    r = numpy.sqrt(numpy.square(averaged_x) +
                                   numpy.square(averaged_y))
    ...
    def bin_lsts(self, sigclip=False):
        ...
        self.outlst_array = numpy.zeros(
            shape=(self.lst_range, len(self.visibility_dict.keys())))
        ...
        for i, date in enumerate(sorted(self.visibility_dict.keys())):
            ...
            for lst in range(self.lst_range):
                ...
                self.outlst_array[lst, i] = (lst_index % 1) * 24
...
class LST_Alignment:
    ...
    def __align_visibilities(self, reference_lst, visibilities):
        ...
        lst_array = numpy.zeros(
            shape=(len(visibilities.keys()), len(reference_lst)))
        i = 0
        ...
        for i, date in enumerate(reversed(self.date_set)):
            if i == 0:
                ...
                offset_array[i] = offset_array[i-1] - \
                    (self.delta_ind - self.delta_rem) * date_delta
    ...
    def align_timestamps(self):
        ...
        # print(lst_ref)
        # We interpolate to the LST's from the first day of observations.
...
class Julian_Parse:
    ...
    def __build_visibility_dict(self, datestamps, filepaths):
        ...
        for datestamp in datestamps:
            ...
            # Holy shit this just works? How awesome is that.
            file_dict[datestamp] = sorted(file_dict[datestamp])
        # print(file_dict)
        return(file_dict)
...
'''))
    command.add_argument('filepath', help=(
        'Directory of .npz files generated from heracasa'))
    command.add_argument('working_directory', help=(
        'where aligned .npz files will be put.'))
